...
export const FREE_TIER = {
  ...
  // 2026-06-08 (round 3) — 最后一条改成显式区分各端的免费时长:
  // iOS / 插件免费档永久,桌面客户端是 24h 试用 → 试用结束要买断或订阅
  // 才能继续用桌面。原来的"三端共用"会让看到首页"买断"卡片的桌面用户
  // 误以为"我下个桌面试试看,既然免费档也支持桌面就可以用着"——实际
  // 24h 后桌面会锁。3 这条同时去掉"三端同步"措辞防止再混淆。
  features: [
    '全部核心功能即装即用（双语字幕 / AI 标黄 / 跟读 / 卡片测验 / AI 口语陪练）',
    '一次性 200K token AI 体验额度，约够 AI 解析 4-5 个新视频',
    '可上传 3 个视频到云端保存，iOS / 插件随时同步打开',
    '单个视频最大可上传 100MB / 20 分钟（一段 TED 短讲或 vlog 够用）',
    '可长按字幕收藏 50 条短语到云端，体验卡片测验和 AI 口语陪练的节奏',
    'iOS / 浏览器插件免费档永久可用；桌面客户端首次免费试用 24 小时',
  ],
} as const;
