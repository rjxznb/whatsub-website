...
└── src/
    ...
    │   ├── page.tsx            # Renders Nav + HeroBenefitLedV2A + DemoDiagonal + Download + CombinedPricing + FAQ + BrandSignOff + Footer. Hero was the Caveat-signature HeroSlim until 2026-06-27 — replaced with the benefit-led pain-question + SubtitleMockup carousel layout; the Caveat brand wordmark moved to BrandSignOff at the bottom
    │   ├── mobile/
    ...
    │   ├── HeroBenefitLedV2A.tsx # Home-page Hero (2026-06-27). Two parallel 「你是否…」pain questions with accent-blue + glow on the key phrases that correspond to the buyer's actual wishes (下载下来逐句精读 / 构建一个属于自己的语料库 / 云端同步), followed by an answer line "whatSub 把这件事做成了一个工具 —— 多平台视频下载解析、AI 自动标出重点表达、云端同步桌面/浏览器/iOS" + Win/macOS download CTAs + Clock-icon「下载即享 24 小时**免费试用** · 满意再付款」trust badge (the「免费试用」is accent-blue + glow). Below the trust badge sits a SubtitleMockup carousel (Mac-window mockup + 7 macOS-client screenshots auto-rotating every 3.5s). Replaced the Caveat-signature HeroSlim — that brand wordmark now lives in BrandSignOff at the page bottom
    │   ├── HeroPluginBenefitLed.tsx # /plugin Hero, same pain-question pattern scoped to the browser-extension audience (实时双语字幕 / 一键划词 / 属于自己的语料库 / 接着翻阅复习). CTAs: Edge 加载项 + Chrome 应用商店. Trust signal: 完全免费 in accent + Sparkles icon (browser plugin is fully free, no trial concept needed). Passed to PluginShowcase via Hero prop
    │   ├── HeroMobileBenefitLed.tsx # /mobile Hero, scenario set to「通勤、躺床上刷 YouTube」+「想构建自己的语料库」. Q1 covers BOTH mobile-direct parse (share-extension on the phone for caption-available YouTube videos) AND continuing-from-desktop. Q2 covers fragment-time corpus review synced across all 3 ends. CTAs: App Store 下载 + 看桌面客户端. Trust signal: 免费下载 in accent + Sparkles icon. Passed to MobileShowcase via Hero prop
    │   ├── SubtitleMockup.tsx  # Carousel mockup mounted inside HeroBenefitLedV2A on /. Dark navy gradient outer card (cohesive with --bg #000000, not the warm-pastel that read as foreign) + accent-blue and amber glow halos + dark macOS-window frame (traffic-light dots + monospace whatSub label) + aspect-video screenshot pane that crossfades through `/screenshots/app/screen-{1..7}.jpg` every 3.5s (700ms fade). A Caveat handwritten label at the card's TOP CENTER (z-30, always on top) crossfades in sync, naming what's on the current screenshot (一键下载多平台视频 / AI 精讲视频内容 / 划词进自己的词汇本 / 云端语料库 多端同步 / 角色扮演练对话 / AI 助手自动调工具 / 内置工具 一句话总结). Hover pauses; dots indicator at the window bottom for manual nav
    │   ├── BrandSignOff.tsx    # Caveat "hey, what'Sub?" signature mounted between the last content section and Footer on /, /plugin, /mobile. Same typography as the old Hero signature (font-display Caveat, clamp(48px, 13vw, 180px), Sub in accent blue with glow + hover-scale), so the brand asset itself is unchanged — only its page position shifts. Lets each page LEAD with pain/value and only see the playful brand wordmark after the pitch lands
    ...
    │   ├── PluginShowcase.tsx  # /plugin page content — accepts optional `Hero` prop (default: inline Caveat-signature Hero block; production passes HeroPluginBenefitLed). Below the Hero slot: 3-feature gallery (双语字幕 / 划词收藏 / 多端同步语料库) + 5-step quick start (incl. AI API key config) + 3-scenario card + 6 FAQ rows + cross-promo final CTA (看桌面客户端 + 看移动端). Both Edge and Chrome buttons are live store links since 2026-06-10
    │   ├── MobileShowcase.tsx  # /mobile page content — accepts optional `Hero` prop (default: inline Caveat-signature Hero block; production passes HeroMobileBenefitLed). Below the Hero slot: 4-feature gallery (导入+精读 / 词汇本 / 多端语料库 / 云端同步视频) + 3-step "怎么用起来" sequence + 3-scenario card + 5 FAQ rows + cross-promo final CTA (App Store + 了解浏览器插件)
    │   ├── Icons.tsx           # Custom inline SVGs for WindowsLogo + AppleLogo (not in lucide); imported by Download, HeroBenefitLedV2A, HeroMobileBenefitLed, MobileShowcase, ComingSoon
...
5. **5 sections + BrandSignOff + 3 dedicated subpages, no SPA routing.** The home `page.tsx` is a single scrollable: HeroBenefitLedV2A (pain-question + SubtitleMockup carousel) → DemoDiagonal → Download (incl. "现已支持" 3-card grid) → CombinedPricing → FAQ → BrandSignOff (Caveat sign-off) → Footer. Nav menu items are anchor scrolls within home; "平台与集成" dropdown navigates to the 3 dedicated subpages (`/plugin`, `/mobile`, `/platforms`). The Caveat "hey, what'Sub?" wordmark was the homepage Hero until 2026-06-27, when the page was reordered to LEAD with pain/value (HeroBenefitLedV2A) and END with the playful brand wordmark (BrandSignOff). Earlier iterations had separate WhyCards / HowSteps / FeatureGrid sections — collapsed into DemoDiagonal once we admitted "show, don't tell" works better than 3 stacked grids of marketing copy when the product is visual. ComingSoon was removed 2026-06-10 when iOS shipped — only Mac App Store + Android remained, not section-worthy on their own.
...
| Task | File |
|------|------|
| Tweak Hero copy (Q1/Q2 questions / answer line / 24h-trial badge / mockup carousel) | Home Hero lives in 3 files: `src/components/HeroBenefitLedV2A.tsx` (text layout + CTAs + trust badge), `src/components/SubtitleMockup.tsx` (carousel of 7 macOS screenshots + Caveat labels), `src/components/BrandSignOff.tsx` (Caveat sign-off at page bottom). Swap a screenshot → drop new JPG in `public/screenshots/app/screen-N.jpg` and update SCREENSHOTS array's matching entry (label + alt). Trust-badge text lives at the end of HeroBenefitLedV2A. Plugin/mobile use parallel `HeroPluginBenefitLed.tsx` / `HeroMobileBenefitLed.tsx` — same pain-question pattern, different scope |
